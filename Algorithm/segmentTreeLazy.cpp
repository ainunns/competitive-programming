...
struct segmentTreeLazy{
	struct node{ // nodenya apa aja
		// EXAMPLE HERE: RANGE ADD UPDATE RANGE SUM QUERY
		int sum;
		int lazy; // lazy propagation
		node(int val = 0) : sum(val), lazy(0){}
	} identity;
	...
	void prop(int nd, int cl, int cr){
		if(st[nd].lazy != 0){
			st[nd].sum += (cr - cl + 1) * st[nd].lazy;
			if(cl != cr){
				st[lc].lazy += st[nd].lazy;
				st[rc].lazy += st[nd].lazy;
			}
			st[nd].lazy = 0;
		}
	}
